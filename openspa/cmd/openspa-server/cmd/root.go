...
import (
	...
	"github.com/greenstatic/openspa/openspa/internal/server"
...
var rootCmd = &cobra.Command{
	Use:   "openspa-server",
	Short: "OpenSPA server to receive OpenSPA request packets",
	Long: `OpenSPA server allows to receive OpenSPA request packets to 
open firewall ports that the OpenSPA client requests.`,

	Run: func(cmd *cobra.Command, args []string) {
		if ver {
			fmt.Printf("OpenSPA server version: %s\n", server.Version)
