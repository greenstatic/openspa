...
func (header *Header) Encode() ([]byte, error) {
	...
	if !CipherSuiteIsSupported(header.CipherSuiteId) {
		return nil, ErrCipherSuiteNotSupported{header.CipherSuiteId}
	}
...
func (header *Header) Equal(h Header) bool {
	...
	if header.CipherSuiteId != h.CipherSuiteId {
		return false
	}
...
func HeaderDecode(data []byte) (Header, error) {
	...
	if !CipherSuiteIsSupported(header.CipherSuiteId) {
		return Header{}, ErrCipherSuiteNotSupported{header.CipherSuiteId}
	}
...
func headerMarshal(h Header) ([]byte, error) {
	...
	buffer[0x2] = h.CipherSuiteId.Bin()
...
func headerUnmarshal(data []byte) (Header, error) {
	...
	h := Header{
		...
		CipherSuiteId: CipherSuiteId(data[0x2]),
	}
