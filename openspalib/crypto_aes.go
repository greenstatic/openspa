...
// Encrypts the plaintext using the key with AES 256 bit CBC encryption. Beware, this function WILL NOT pad any data,
// if the plaintext is not a multiple of the block size (16 bytes) we will return an error. We will however create a
// random IV (16 bytes) and add it as the prefix of the ciphertext.
//
...
// Decrypts the plaintext that was encrypted using AES 256 bit CBC encryption, using the key. Beware, this function
// WILL NOT attempt to remove any padding, that is left for the user to do. Note this function assumes that the IV that
// was used to encrypt the plaintext is stored as the prefix of the ciphertext (first 16 bytes).
//
// Inspired by the example at: https://golang.org/pkg/crypto/cipher/#NewCBCDecrypter
func aes256CbcDecrypt(ciphertext []byte, key []byte) ([]byte, error) {
...
func aes256CbcEncryptWithPadding(plaintext []byte, key []byte) ([]byte, error) {
	...
	// Generate random IV
	iv, err := aes256CbcRandomIV()
	if err != nil {
		return nil, err
	}

	return aes256CbcEncrypt(dataPadded, key, iv)
}

// Calls AES_256_CBC_decrypt() and returns the result without the PKCS7 padding.
func aes256CbcDecryptWithPadding(ciphertext []byte, key []byte) ([]byte, error) {
	plaintextPadded, err := aes256CbcDecrypt(ciphertext, key)
	if err != nil {
		return nil, err
	}
...
func aes256CbcRandomIV() ([]byte, error) {
